...
//void AnalyseParticles(LHEF::Reader *reader, ExRootTreeBranch *branch)
{
  ...
  if( Lepton4_TL.M()                          <= 300 ){return 0;}
  if( Lepton4_TL.M() >  300 ){return 1;}
  return -1;
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc != 4)
  {
    cout << " Usage: " << appName << " input_file" << " outHead_file" << " outTail_file"<<endl;
    cout << " input_file - input file in LHEF format," << endl;
    cout << " outHea_file - output file with m(H) <300 GeV." << endl;
  ...
  ofstream headFile(argv[2]);
  ofstream tailFile(argv[3]);
  LHEF::Writer *headWriter   = new LHEF::Writer(headFile);
  LHEF::Writer *tailWriter     = new LHEF::Writer(tailFile);
  headWriter  ->heprup = inputReader->heprup;
 ...
 // ExRootTreeBranch *branchParticle = treeWriter->NewBranch("Particle", TRootLHEFParticle::Class());
  ...
  if(allEntries > 0)
  {
    ...
    while(inputReader->readEvent())
    {
      ...
      {
        headWriter->hepeup = inputReader->hepeup;
        headWriter->writeEvent();
      }
      if( area == 1 ){
