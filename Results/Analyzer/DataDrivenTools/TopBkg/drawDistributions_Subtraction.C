...
// drawDistributions
//------------------------------------------------------------------------------
void drawDistributions_Subtraction(Int_t    njet       = 0,
		       TString  channel    = "All",
		   ...
		   Bool_t   moveOverflow = true)
{
  ...
  if (Top_Yield!=0){ 
	  cout <<Form("%s : subData_Yield = %.2f, Top_Yield = %.2f, Scale Factor = %.3f",hname.Data(),subData_Yield,Top_Yield,subData_Yield/Top_Yield) <<endl;
  ...
  canvas->SaveAs(Form("%s/%s%s_subtraction.%s",
		      _output.Data(),
		      hname.Data(),
		      suffixLogy.Data(),
		      _format.Data()));
