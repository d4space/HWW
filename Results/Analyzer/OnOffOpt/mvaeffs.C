...
void StatDialogMVAEffs::DrawHistograms() 
{
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      if (info->maxSignificanceErr > 0) {
         info->line2 = tl.DrawLatex( 0.15, 0.15, Form("%5.2f +- %4.2f when cutting at %6.3f SigE=%6.3f BkgE=%6.3f", 
                                                      info->maxSignificance, 
                                                      info->maxSignificanceErr, 
                                                      info->sSig->GetXaxis()->GetBinCenter(maxbin),
	     					info->sigE->GetBinContent(maxbin),
	     					info->bgdE->GetBinContent(maxbin)) );
      }
      else {
         info->line2 = tl.DrawLatex( 0.15, 0.15, Form("%4.2f when cutting at %6.3f SigE=%6.3f BkgE=%6.3f", 
                                                      info->maxSignificance, 
                                                      info->sSig->GetXaxis()->GetBinCenter(maxbin),
	     					info->sigE->GetBinContent(maxbin),
	     					info->bgdE->GetBinContent(maxbin)) );
...
void StatDialogMVAEffs::PrintResults( const MethodInfo* info )
{
   ...
   if (info->line2 !=0 ) {
      if (info->maxSignificanceErr > 0) {
         info->line2->SetText( 0.15, 0.15, Form("%3.2g +- %3.2g when cutting at %3.2g SigE=%6.3f BkgE=%6.3f", 
                                                info->maxSignificance, 
                                                info->maxSignificanceErr, 
                                                info->sSig->GetXaxis()->GetBinCenter(maxbin),
	     					info->sigE->GetBinContent(maxbin),
         ...
         info->line2->SetText( 0.15, 0.15, Form("%3.4f when cutting at %3.4f SigE=%6.3f BkgE=%6.3f",
	                                        info->maxSignificance, 
                                                info->sSig->GetXaxis()->GetBinCenter(maxbin),
	     					info->sigE->GetBinContent(maxbin),
	     					info->bgdE->GetBinContent(maxbin)) );
