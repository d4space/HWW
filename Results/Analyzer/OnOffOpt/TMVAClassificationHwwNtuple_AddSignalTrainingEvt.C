...
//   std::cout << "--- TMVAClassification       : Using input file: " << input->GetName() << std::endl;
   ...
   //     for (UInt_t ivar=0; ivar<4; ivar++) signal->SetBranchAddress( Form( "var%i", ivar+1 ), &(treevars[ivar]) );
        SB_Chain->SetBranchAddress("mWW",&(treevars[0]) );
        SB_Chain->SetBranchAddress("pt1",&(treevars[1]) );
        SB_Chain->SetBranchAddress("pt2",&(treevars[2]) );
        SB_Chain->SetBranchAddress("pfmet",&(treevars[3]) );
        SB_Chain->SetBranchAddress("mpmet",&(treevars[4]) );
        SB_Chain->SetBranchAddress("dphill",&(treevars[5]) );
        SB_Chain->SetBranchAddress("mll",&(treevars[6]) );
        SB_Chain->SetBranchAddress("ptll",&(treevars[7]) );
        SB_Chain->SetBranchAddress("channel",&(treevars[8]) );
        SB_Chain->SetBranchAddress("baseW",&(treevars[9]) );
        SB_Chain->SetBranchAddress("trigger",&(treevars[10]) );
        SB_Chain->SetBranchAddress("nextra",&(treevars[11]) );
        SB_Chain->SetBranchAddress("zveto",&(treevars[12]) );
        SB_Chain->SetBranchAddress("sameflav",&(treevars[13]) );
        SB_Chain->SetBranchAddress("njet",&(treevars[14]) );
        SB_Chain->SetBranchAddress("dymva1",&(treevars[15]) );
        SB_Chain->SetBranchAddress("dphilljetjet",&(treevars[16]) );

	double channel, baseW, Ntry, mWW, trigger, pt1, pt2, nextra;
	double pfmet, zveto, sameflav, njet, dymva1, mll, mpmet;
	double dphilljetjet, bveto_mu, bveto_ip, nbjettche, ptll;
	Ntry = SB_Chain->GetEntries();
        for (UInt_t i=0; i<Ntry; i++) {
	  SB_Chain->GetEntry(i);
          for (UInt_t ivar=0; ivar<10; ivar++) vars[ivar] = treevars[ivar];
	  //cout<<"baseW: "<<baseW<<endl;
	  // EMu Channel
	  if( channel != 2 && channel != 3) continue;
	  if(CommonCut(trigger, pt1, pt2, nextra) != 1) continue;
	  cout<<"Pass CommonCut"<<endl;

          if(i % 2 == 0) // Train
	  {
	    //cout<<"event: "<<i<<endl;
	    //cout<<"mWW: "<<mWW<<endl;
	    if( mWW > 160)factory->AddSignalTrainingEvent( vars, baseW );
	    else factory->AddBackgroundTrainingEvent( vars, baseW);
	  }
	  else // Test
	  {
	    if( mWW > 160)factory->AddSignalTestEvent( vars, baseW );
	    else factory->AddBackgroundTestEvent( vars, baseW );
	  }
   //        signal->GetEntry(i);
